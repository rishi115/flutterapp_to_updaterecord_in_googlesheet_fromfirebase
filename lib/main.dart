import 'dart:async';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:firebase_core/firebase_core.dart';


import 'package:flutter/material.dart';
import 'package:googleapis/sheets/v4.dart';
import 'package:googleapis_auth/auth_io.dart';



final _credentials = ServiceAccountCredentials.fromJson(r'''
{
  "type": "service_account",
  "project_id": "gsheet-384920",
  "private_key_id": "f289d37a745a002d64b6f4f3e3ac4e9d19a02a02",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  "client_email": "gsheet@gsheet-384920.iam.gserviceaccount.com",
  "client_id": "100810567459987202357",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/gsheet%40gsheet-384920.iam.gserviceaccount.com"
}
''');

void main() async{
  WidgetsFlutterBinding.ensureInitialized();
  await Firebase.initializeApp();

  runApp(MyApp());
}

class MyApp extends StatelessWidget {
  @override

  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'My Attendance App',
      home: AttendanceScreen(),
    );
  }
}

class AttendanceScreen extends StatefulWidget {
  @override
  _AttendanceScreenState createState() => _AttendanceScreenState();
}

class _AttendanceScreenState extends State<AttendanceScreen> {
  String _spreadsheetId = '1UR4fyNtd-vvvRQPDDBIJzMYquNp7H4NcU11T6qwh1gc';
  String getColumnName(int colIndex) {
    final int asciiA = 'A'.codeUnitAt(0);
    final int asciiZ = 'Z'.codeUnitAt(0);
    final int lettersCount = asciiZ - asciiA + 1;
    String result = '';
    while (colIndex > 0) {
      final int remainder = (colIndex - 1) % lettersCount;
      result = '${String.fromCharCode(asciiA + remainder)}$result';
      colIndex = (colIndex - 1) ~/ lettersCount;
    }
    return result;
  }

  @override
  void initState() {
    super.initState();
    _getStudentNames();
  }
  List<String> _studentNames = [];

  Future<void> _addAttendance(String name, bool present) async {
    final client = await clientViaServiceAccount(_credentials, ['https://www.googleapis.com/auth/spreadsheets']);
    final sheetsApi = SheetsApi(client);
    final sheetName = 'Users';
    final now = DateTime(2002,8,9);
    final date = '${now.month}/${now.day}/${now.year}';
    final range = '$sheetName!A:Z';
    final response = await sheetsApi.spreadsheets.values.get(_spreadsheetId, range);
    final values = response.values;

    if (values == null || values.isEmpty) {
      // If the sheet is empty, add the header row
      final headerValues = [['Name', date]];
      final headerBody = ValueRange(values: headerValues);
      await sheetsApi.spreadsheets.values.append(headerBody, _spreadsheetId, range, valueInputOption: 'USER_ENTERED');
    } else {
      // Find the header row by searching for the row that contains the 'Name' column
      final headerRow = values.firstWhere((row) => row.contains('Name'), orElse: () => []);

      if (!headerRow.contains(date)) {
        // If the date is not present in the header row, add a new column
        final lastCol = headerRow.length;
        final headerValues = [['', date]];
        final headerBody = ValueRange(values: headerValues);
        await sheetsApi.spreadsheets.values.update(
          headerBody,
          _spreadsheetId,
          '$sheetName!${getColumnName(lastCol + 1)}1:${getColumnName(lastCol + 1)}1',
          valueInputOption: 'USER_ENTERED',
        );
      }
    }

    // Get the updated values after updating the header row
    final updatedResponse = await sheetsApi.spreadsheets.values.get(_spreadsheetId, range);
    final updatedValues = updatedResponse.values;

    final row = updatedValues?.indexWhere((element) => element[0] == name) ?? -1;
    if (row != -1) {
      final col = updatedValues![0].indexOf(date);
      final rowValues = updatedValues[row];
      rowValues[col] = present ? 'Present' : 'Absent';
      final body = ValueRange(values: [rowValues]);
      await sheetsApi.spreadsheets.values.update(
        body,
        _spreadsheetId,
        '$sheetName!A${row + 1}:${getColumnName(col + 1)}${row + 1}',
        valueInputOption: 'USER_ENTERED',
      );
    } else {
      final values = [[name]];
      values[0].addAll(List.filled(2, ''));
      values[0][1] = present ? 'Present' : 'Absent';

      final body = ValueRange(values: values);
      await sheetsApi.spreadsheets.values.append(body, _spreadsheetId, range, valueInputOption: 'USER_ENTERED');
    }
  }









  Future<void> _getStudentNames() async {
    final studentNames = await FirebaseFirestore.instance.collection('students').get().then((QuerySnapshot querySnapshot) {
      final List<String> names = [];
      querySnapshot.docs.forEach((doc) {
        final name = (doc.data() as Map<String, dynamic>)['name'];
        if (name != null) {
          names.add(name);
        }


      });
      return names;
    });
    setState(() {
      _studentNames = studentNames;
    });
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        toolbarHeight: 65,

        title: Text('Attendance'),
      ),
      body: ListView(
        children: [
          for (var name in _studentNames)
            AttendanceCard(name: name, onPresent: () => _addAttendance(name, true), onAbsent: () => _addAttendance(name, false)),
        ],
      ),
    );
  }
}

class AttendanceCard extends StatelessWidget {
  final String name;
  final VoidCallback onPresent;
  final VoidCallback onAbsent;

  AttendanceCard({required this.name, required this.onPresent, required this.onAbsent});

  @override
  Widget build(BuildContext context) {
    return Container(

      child: Card(
        child: ListTile(
          title: Text(name),
          trailing: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              IconButton(
                icon: Icon(Icons.check),
                onPressed: onPresent,
              ),
              IconButton(
                icon: Icon(Icons.close),
                onPressed: onAbsent,
              ),
            ],
          ),
        ),
      ),
    );
  }
}
